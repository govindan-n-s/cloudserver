...
const objectLayerClientStub = new interface.ObjectLayer('localhost:50051', grpc.credentials.createInsecure());
...
class HclClient {
    ...
    const log = createLogger("tempRequestId");
    logHelper(log, 'info', 'put object data received by hcl backend ' , objectLayerClientStub ,
                    this._client);
     toArray(stream, function (err, arr) {
       const call = objectLayerClientStub.PutObject((err,response)=>{
        console.log(' object put status ', response , err);
       });
       call.write({metadata:{objname:{name:"object1"}, len: arr.length}});
       call.write({content:{datablock:Buffer.from(arr)}});
       call.end();
     });
